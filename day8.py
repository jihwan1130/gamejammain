    ...
    def update_typewriter(self, logs):
        ...
        if self.type_sound:
            try:
                ...
                self.type_sound.set_volume(vol * 0.4)
                
                if is_actively_typing:
                    if self.type_sound_channel is None or not self.type_sound_channel.get_busy():
                        self.type_sound_channel = pygame.mixer.find_channel(True)
                        if self.type_sound_channel:
                            self.type_sound_channel.play(self.type_sound)
                else:
                    if self.type_sound_channel and self.type_sound_channel.get_busy():
