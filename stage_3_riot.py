...
class RiotPacificationGame:
    ...
    def on_fail(self):
        ...
        if resources_game and hasattr(resources_game, 'my_crew'):
            if "정치인" in resources_game.my_crew:
                self.has_politician = True
        
        # 정치인이 없다면 자동으로 정신력 -50 페널티 설정
        if not self.has_politician:
    ...
    def apply_penalty(self):
        ...
        if self.penalty_selected == 1:
            if hasattr(resources_game, 'my_crew') and "정치인" in resources_game.my_crew:
                resources_game.my_crew.remove("정치인")
                print("[PENALTY] Politician (정치인) exiled.")
    ...
    def draw(self, surface):
        ...
        if self.state != "PLAYING":
            ...
            if self.state == "SUCCESS":
                ...
                msg = self.font_main.render("🚨 설득 실패 (FAIL) 🚨", True, (230, 50, 50))
                virtual_surf.blit(msg, (500 - msg.get_width()//2, 260))
                
                if not self.has_politician:
                    lbl_warning = self.font_main.render("정치인이 존재하지 않아 난동이 드세졌습니다.", True, (255, 100, 100))
                    lbl_subwarning = self.font_sub.render("페널티: 정신력 -50  |  " + sub_text, True, (200, 200, 200))
                    ...
                    if self.penalty_selected is None:
                        ...
                        selected_name = "정치인 추방" if self.penalty_selected == 1 else "정신력 -50"
                    ...
                    pygame.draw.rect(virtual_surf, (230, 50, 50), btn1_rect, 2 if self.penalty_selected == 1 else 1)
                    
                    b1_title = self.font_main.render("1. 정치인 추방", True, (255, 200, 200))
                    b1_desc = self.font_sub.render("추방: [정치인]", True, (240, 100, 100))
                    ...
                    pygame.draw.rect(virtual_surf, COLOR_THEME, btn2_rect, 2 if self.penalty_selected == 2 else 1)
                    
                    b2_title = self.font_main.render("2. 정신적 타격", True, (200, 200, 255))
                    b2_desc = self.font_sub.render("정신력 -50", True, (150, 150, 250))
